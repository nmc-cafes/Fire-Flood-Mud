...
dat <- dat_raw %>%
  mutate(high_severity = if_else(severity %in% c(3,4), 1, 0)) %>%
  mutate(high_severity = factor(high_severity))

set.seed(47)
dat_split <- initial_split(dat, strata = high_severity)
dat_train <- training(dat_split)
dat_test <- testing(dat_split)

cls_metrics <- metric_set(roc_auc, accuracy, sensitivity, specificity)

cls_formula <- formula(high_severity ~ 
                         mass_burnt_pct +
