...
states <- data.frame("state"=rep("WA",2),
                     "fire"=fires)
fire_list <- list()
for(i in 1:length(fires)){
  ...
  for(j in 1:nrow(sites)){
    sites$site_name[j] <- paste0(substr(fires[i], start = 1, stop = 3),j,"_COR")
  }
  fire_list[[i]] <- sites
}
...
write.csv(sample_sites_df, here("Sample_Basins_corrected.csv"), row.names=F)
